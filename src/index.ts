#!/usr/bin/env node
/**
 * Granola MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their own Granola API key as Bearer token.
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Granola AI meeting notes, summaries, transcripts, and attendees through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Granola API key as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <your-granola-api-key>',
      howToGetKey:
        'Generate an API key at https://granola.ai → Settings → API',
    },
    configTemplate: {
      mcpServers: {
        granola: {
          ...
          headers: {
            Authorization: 'Bearer <your-granola-api-key>',
          },
        },
      },
    },
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/granola/',
    },
  });
});
...
<head>
  ...
  <title>Granola MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Granola MCP Server</h1>
    <p class="subtitle">Access Granola AI meeting notes, summaries, transcripts, and attendees through MCP tools.</p>

    <div class="card">
      <h2>How to get your Granola API key</h2>
      <ol class="steps">
        <li>Log in to <a href="https://granola.ai" target="_blank" rel="noopener">Granola</a>.</li>
    ...
    <div class="card">
      ...
      <label for="apiKey">Granola API Key</label>
...
function createMCPServer(client: GranolaClient): Server {
  const server = new Server(
    { name: 'granola-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
<head>
  ...
  <title>Authorize \u2014 Granola MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Granola MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Granola MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Granola MCP HTTP Server running on port ${PORT}`);
  console.log(`  MCP endpoint:   http://localhost:${PORT}/mcp`);
  console.log(`  Health check:   http://localhost:${PORT}/health`);
  console.log(`  Tools:          ${tools.length}`);
  console.log(`  Transport:      Streamable HTTP`);
  console.log(`  Auth:           Bearer passthrough (client provides Granola API key)`);
});
