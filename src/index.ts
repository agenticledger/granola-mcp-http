#!/usr/bin/env node
/**
 * Granola MCP Server — Streamable HTTP, BROKER-FIRST (auth model "B").
 *
 * This MCP holds ZERO Granola secrets. It is a *client* of the Connections Broker
 * (https://connectionsbroker.agenticledger.ai), which vaults each user's Granola
 ...
 *      `Authorization: Bearer <granola-api-key>`, use it directly.
 *   2. Broker-first (default): derive the caller's `principal`, sign a short-lived
 *      JWT, ask the broker POST /token for the Granola key, construct the client.
...
const SLUG = 'granola';
const NAME = 'Granola MCP Server';
const VERSION = '2.0.0';

/** Construct a GranolaClient from a broker-resolved (or raw passthrough) credential. */
function makeClient(credential: string): GranolaClient {
  return new GranolaClient(credential);
}
  ...
  res.json({
    ...
    auth: {
      ...
      alternativeAuth: {
        ...
        description: 'Escape hatch (no secret held): pass a raw Granola API key as Bearer.',
      },
    },
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    authModes: [
      'broker-first (default): resolves the Granola key via the Connections Broker',
      'bearer-passthrough (escape hatch): Authorization: Bearer <granola-api-key>',
    ],
  });
});
...
type ClientResolution =
  | { kind: 'client'; client: GranolaClient }
...
async function connectMessage(principal: string): Promise<string> {
  ...
  if ('error' in started) {
    return `Granola isn't connected for this caller yet, and starting a connection failed: ${started.error}`;
  }
  return JSON.stringify(
    {
      ...
      message:
        brokerProviderKind === 'static'
          ? 'Granola is not connected for this caller yet. Connect it once via the link below (paste your Granola API key into your platform’s broker connect flow), then run the tool again — it will work.'
          : 'Granola is not connected for this caller yet. Open the connect link below once, then run the tool again — it will work.',
  ...
  if (raw) {
    ...
    if (!brokerConfigured) {
      res.status(503).json({
        ...
        alternative: { Authorization: 'Bearer <your-granola-api-key>' },
      });
      return;
    }
